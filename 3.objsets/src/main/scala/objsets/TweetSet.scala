...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  private def maxRetweeted(one: Tweet, two: Tweet): Tweet =
    if (two == null) one
    else if (one == null) two
    else if (one.retweets > two.retweets) one
    else two

  override def mostRetweeted(): Tweet = {
    maxRetweeted(elem, maxRetweeted(left.mostRetweeted, right.mostRetweeted))
  }
...
object GoogleVsApple {
  ...
  lazy val googleTweets: TweetSet = TweetReader.allTweets.filter(by(google))
  lazy val appleTweets: TweetSet = TweetReader.allTweets.filter(by(apple))
  ...
  private def by(keywords: List[String])(tweet: Tweet): Boolean =
